...
bool startSelection(Population *population, std::string target)
{
	...
	for (int i = 0 ; i < population->getNbrElements() ; i++)
	{
		...
		probabilityMappedList.insert(probabilityMappedList.end(), population->_elements[i].getPotential(), i);;
		if (population->_elements[i].getPotential() == pow(target.length(), 2))
		{
			std::cout << population->_elements[i].getDna() << std::endl;
			std::cout << "found in " << nbPopulation << " generations" << std::endl;
			return (true);
		}
	}
...
{
	for (int i = 0 ; i < Child.getDna().length() ; i++)
	{
